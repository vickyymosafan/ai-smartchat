  ...
  return (
    <>
      <div className="p-3 border-t border-sidebar-border">
        ...
        <div 
          className={`mb-3 ${currentTrack?.lyrics ? 'cursor-pointer hover:opacity-80' : ''}`}
      ...
      {showLyrics && (currentTrack?.lyrics || playlist[0]?.lyrics) && (
        ...
        >
          <div 
            className="bg-card border border-border rounded-lg shadow-xl w-[90%] max-w-md max-h-[80vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div>
                <h3 className="font-semibold text-foreground">{currentTrack?.title || playlist[0]?.title}</h3>
                <p className="text-sm text-muted-foreground">{currentTrack?.artist || playlist[0]?.artist}</p>
              </div>
              <Button
                variant="ghost"
                size="icon"
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div className="whitespace-pre-line text-sm text-foreground leading-relaxed pb-4">
